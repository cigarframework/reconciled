...
type stateMap struct {
	...
	dirty map[string]*entryStateMap
...
type readOnlyStateMap struct {
	m       map[string]*entryStateMap
...
func (m *stateMap) Store(key string, value storage.State) {
	...
	if e, ok := read.m[key]; ok {
		...
		if !read.amended {
			...
			m.dirtyLocked()
			m.read.Store(readOnlyStateMap{m: read.m, amended: true})
		}
		m.dirty[key] = newEntryStateMap(value)
	}
	m.mu.Unlock()
}
...
func (e *entryStateMap) unexpungeLocked() (wasExpunged bool) {
	return atomic.CompareAndSwapPointer(&e.p, expungedStateMap, nil)
}
...
func (e *entryStateMap) delete() (hadValue bool) {
	for {
		p := atomic.LoadPointer(&e.p)
		if p == nil || p == expungedStateMap {
			return false
...
func (m *stateMap) Range(f func(key string, value storage.State) bool) {
	...
	if read.amended {
		...
		read, _ = m.read.Load().(readOnlyStateMap)
		if read.amended {
			read = readOnlyStateMap{m: m.dirty}
			m.read.Store(read)
			m.dirty = nil
			m.misses = 0
		}
...
func (m *stateMap) missLocked() {
	m.misses++
	if m.misses < len(m.dirty) {
		return
	}
	m.read.Store(readOnlyStateMap{m: m.dirty})
	m.dirty = nil
	m.misses = 0
}

func (m *stateMap) dirtyLocked() {
	if m.dirty != nil {
		return
	}

	read, _ := m.read.Load().(readOnlyStateMap)
	m.dirty = make(map[string]*entryStateMap, len(read.m))
	for k, e := range read.m {
...
func (e *entryStateMap) tryExpungeLocked() (isExpunged bool) {
	p := atomic.LoadPointer(&e.p)
	for p == nil {
		if atomic.CompareAndSwapPointer(&e.p, nil, expungedStateMap) {
			return true
		}
		p = atomic.LoadPointer(&e.p)
	}
	return p == expungedStateMap
}
